...
(defun kisses--set-local-vars ()
  ...
  (set-window-buffer (selected-window) (get-buffer "*kisses*"))
...
banner at the center. Also checks to see if buffer named *kisses* already exists and if so overwrites it"
  (unless kisses--box-dimensions
    (kisses--banner-box-dimenions))
  (let* ((splash-buffer (get-buffer-create "*kisses*"))
...
(defun kisses-window-size-change-function (arg)
  ...
  ;; get list of windows displaying "*kisses*"
  (when (get-buffer "*kisses*")
    ;; (if (string= major-mode "kisses-mode")
    ;;  (kisses--set-window-start (selected-window)))
    (let ((w-to-update (get-buffer-window-list "*kisses*" nil (selected-frame))))
...
(if (and (not (member "-no-splash"  command-line-args))
    ...
    (progn
      (add-hook 'window-setup-hook 'kisses-initial-buffer)
